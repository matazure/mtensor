...
void bm_hete_tensor_cast(benchmark::State& state) {
    ...
    state.SetBytesProcessed(state.iterations() * ts_dst.size() *
                            (sizeof(decltype(ts_src[0])) + sizeof(decltype(ts_dst[0]))));
}

#define BM_HETE_TENSOR_CAST(ValueTypeSrc, ValueTypeDst, Rank)                          \
    auto bm_hete_tensor_##ValueTypeSrc##_rank##Rank##_cast_##ValueTypeDst =            \
        bm_hete_tensor_cast<HETE_TENSOR<ValueTypeSrc, Rank>, ValueTypeDst>;            \
    BENCHMARK(bm_hete_tensor_##ValueTypeSrc##_rank##Rank##_cast_##ValueTypeDst)        \
        ->RangeMultiplier(bm_config::range_multiplier<ValueTypeDst, Rank, HETE_TAG>()) \
        ->Range(bm_config::min_shape<ValueTypeDst, Rank, HETE_TAG>(),                  \
                bm_config::max_shape<ValueTypeDst, Rank, HETE_TAG>())                  \
        ->UseRealTime();
...
void bm_hete_tensor_section(benchmark::State& state) {
    ...
    state.SetBytesProcessed(state.iterations() * ts_dst.size() *
                            sizeof(typename _Tensor::value_type) * 2);
}

#define BM_HETE_TENSOR_SECTION(ValueType, Rank)                                     \
    auto bm_hete_tensor_##ValueType##_rank##Rank##_section =                        \
        bm_hete_tensor_section<HETE_TENSOR<ValueType, Rank>>;                       \
    BENCHMARK(bm_hete_tensor_##ValueType##_rank##Rank##_section)                    \
...
#define BM_HETE_TENSOR_STRIDE_STEP2(ValueType, Rank)                                \
    auto bm_hete_tensor_##ValueType##_rank##Rank##_stride_step2 =                   \
        bm_hete_tensor_stride_step2<HETE_TENSOR<ValueType, Rank>>;                  \
    BENCHMARK(bm_hete_tensor_##ValueType##_rank##Rank##_stride_step2)               \
        ->RangeMultiplier(bm_config::range_multiplier<ValueType, Rank, HETE_TAG>()) \
        ->Range(bm_config::min_shape<ValueType, Rank, HETE_TAG>(),                  \
                bm_config::max_shape<ValueType, Rank, HETE_TAG>())                  \
        ->UseRealTime();
