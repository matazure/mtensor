...
__global__ void gold_tensor_rank1_mul_kenel(_ValueType* p_dst, _ValueType* p1, _ValueType* p2,
                                            int_t count) {
...
void bm_gold_cu_tensor_rank1_mul(benchmark::State& state) {
    ...
    while (state.KeepRunning()) {
        ...
        gold_tensor_rank1_mul_kenel<<<policy.grid_size(), policy.block_dim(),
                                      policy.shared_mem_bytes(), policy.stream()>>>(
            ts_re.data(), ts0.data(), ts1.data(), ts_re.size());
...
#define BM_GOLD_CU_TENSOR_RANK1_MUL(ValueType)                                               \
    ...
    BENCHMARK(bm_gold_cu_tensor_##ValueType##_rank1_mul)                                     \
...
#define BM_GOLD_HOST_TENSOR_RANK1_MUL(ValueType)                                                 \
    ...
    BENCHMARK(bm_gold_host_tensor_##ValueType##_rank1_mul)                                       \
        ->RangeMultiplier(bm_config::range_multiplier<ValueType, 1, host_tag>())                 \
        ->Range(bm_config::min_shape<ValueType, 1, host_tag>(),                                  \
                bm_config::max_shape<ValueType, 1, host_tag>())                                  \
        ->UseRealTime();
...
#define BM_HETE_TENSOR_BINARY_OPERATOR(OpName, ValueType, Rank)                     \
    auto bm_hete_tensor_##ValueType##_rank##Rank##_##OpName =                       \
        bm_hete_tensor_##OpName<HETE_TENSOR<ValueType, Rank>>;                      \
    BENCHMARK(bm_hete_tensor_##ValueType##_rank##Rank##_##OpName)                   \
