...
void bm_cu_prod_block(benchmark::State& state) {
	...
	cuda::matrix<_ValueType> ts_re(ext);
	while (state.KeepRunning()) {
		cuda::numeric::prod_block<32>(ts_lhs, ts_rhs, ts_re);
		cuda::device_synchronize();
	}
...
BENCHMARK_TEMPLATE1(bm_cu_prod_block, float)->RangeMultiplier(2)->Range(1 << 10, 1 << 12)->UseRealTime();
BENCHMARK_TEMPLATE1(bm_cu_prod_block, double)->RangeMultiplier(2)->Range(1 << 10, 1 << 11)->UseRealTime();
