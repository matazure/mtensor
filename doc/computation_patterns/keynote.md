...
很卷积多神经网络里卷积运算是多通道的, 我们可以通过向量化指令集来加速他们. 在mtensor中其实现非常简单, 将tensor的value_type特化为simd类型就好, 只需添加一行代码.
...
除了在cpu, 很多时候我们希望我们的程序运行在gpu上, mtensor中只需要使用cuda::tensor就可以切换到cuda了
...
前面的小节阐述了卷积运算如何快速向量化和并行化, 下面看下卷积如后接stride和relu6
