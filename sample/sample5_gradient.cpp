...
int main(int argc, char *argv[]) {
	...
	tensor<byte, 2, padding_layout<2>> img_padding(img_gray.shape(), padding, padding); //构造有padding的图像tensor
	for_border(img_padding.shape(), padding, padding, [=](pointi<2> idx) {
		img_padding(idx) = 0;
	}); //将边界置零

	for_index(img_padding.shape(), [=](pointi<2> idx){
		img_padding(idx) = img_gray(idx);
	});
