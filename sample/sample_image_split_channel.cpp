...
int main(int argc, char* argv[]) {
    ...
    //减均值
    auto img_mean = view::cast<point3f>(img_gray) - point3f{128, 128, 128};
    //转为float类型，并归一化
    auto img_scale = img_mean * point3f{0.0039f, 0.0039f, 0.0039f};

    auto width = img_gray.shape(0);
    auto height = img_gray.shape(1);
    tensor3f img_split(width, height, 3);
    //在img_split的基础上获取各个通道
    auto img_split_r_view = view::unstack<2>(img_split, 0);
    auto img_split_b_view = view::unstack<2>(img_split, 1);
    auto img_split_g_view = view::unstack<2>(img_split, 2);
